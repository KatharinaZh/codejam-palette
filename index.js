...
let pencilLine = document.querySelector('#pencil');
let fillLine = document.querySelector('#fill_bucket');
let pipetteLine = document.querySelector('#pipette');
let settingsPanel = document.querySelector('#settings_panel');

let currentColor = document.querySelector('#current_color');
let prevColor = document.querySelector('#prev_color');
let redColor = document.querySelector('#red_color');
let blueColor = document.querySelector('#blue_color');
...
//выбор инструмента по клику на панель инструментов
settingsPanel.addEventListener('click', (event) => {
  let tool = event.target.closest('.settings_panel--item');
  let elements = [];
  document.querySelectorAll('.settings_panel--item').forEach(tool => tool.classList.remove('active_tool')); //убираем выделение цветом со всех инструментов
  elements.push(tool);
...
function drawWithPencil() {
  ...
  canvas.addEventListener("mousedown", function(e){
    if (pencilLine.classList.contains('active_tool')) {
      ...
      isMouseDown = true;
      ctx.beginPath(); //чтобы не соединялись линии при повторном нажатии
...
red_color.addEventListener("click", addPredefColor);
blue_color.addEventListener("click", addPredefColor);

function addPredefColor() {
...
function getColor() {
  canvas.addEventListener("mousedown", function(e) {
    if (pipetteLine.classList.contains('active_tool')){
      ...
      let x = e.pageX - pos.x;
      let y = e.pageY - pos.y;
      let canv = this.getContext('2d');
      let  p = canv.getImageData(x, y, 1, 1).data;
